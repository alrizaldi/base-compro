...
export async function GET(request: NextRequest) {
  try {
    ...
    const headers = ["ID", "Name", "Role", "Content", "Rating", "Avatar URL", "Created At"];
    ...
    ws["!cols"] = [{ wch: 26 }, { wch: 20 }, { wch: 20 }, { wch: 60 }, { wch: 10 }, { wch: 40 }, { wch: 25 }];
  ...
  try {
    ...
    for (let i = 0; i < dataRows.length; i++) {
      ...
      try {
        ...
        if (existing) {
          await Testimonial.updateOne({ _id: id }, { name, role, content, rating, avatar });
          results.updated++;
        } else {

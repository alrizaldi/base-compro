...
function paginateData<T>(data: T[], page: number, limit: number): PaginatedResponse<T> {
...
export async function fetchArticles(
  ...
  if (USE_MOCK) {
    ...
    let filtered = mockArticlesData.filter((a) => a.published);
    if (params?.search) {
...
export async function fetchArticleById(id: string): Promise<Article | null> {
  if (USE_MOCK) {
    await new Promise((r) => setTimeout(r, 300));
    return mockArticlesData.find((a) => a.id === id) || null;
  }
...
export async function updateArticle(
  ...
  if (USE_MOCK) {
    ...
    mockArticlesData[index] = { ...mockArticlesData[index], ...data, updatedAt: new Date().toISOString() };
